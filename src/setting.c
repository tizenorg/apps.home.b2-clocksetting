...
/*
	if( index == LANGUAGE_MENU_INDEX ) {
		if( is_connected_GM() ) {
			snprintf(buf, sizeof(buf)-1, "%s", LANGUAGE_ICON_DISABLED);
		} else {
			snprintf(buf, sizeof(buf)-1, "%s", LANGUAGE_ICON_ENABLED);
		}
	} else {*/
		if(is_emergency)
...
static Evas_Object* _create_mainlist_winset(Evas_Object* parent, appdata* ad)
{
	...
	for( idx = 0; idx < item_count-1; idx++ )
	{
		...
		/*
		if( !is_emergency && ( idx == LANGUAGE_MENU_INDEX ) )
		{
			lang_item = id->item;
		}
		*/
...
bool app_create(void *data)
{
	...
	//appcore_set_event_callback(APPCORE_EVENT_LANG_CHANGE, _lang_changed, ad);
